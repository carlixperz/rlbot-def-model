 ...
 checkArgs(msg, msgArgs) {
  ...
  if (this.args != undefined) {
   if (msgArgs.length == 0 && this.args.find(x => !x.optional) != undefined) {
    util.getSend(msg, 'Se necesita un argumento');
    return false;
   }
   let argsPos = 0
...
module.exports = {
 ...
 loadFile: function (path) {
  ...
  for (var module of cmds) {
   var files = fs.readdirSync(`./cmds/${module}`);
   for (var file of files) {
    if (fs.statSync(`./cmds/${module}/${file}`).isFile()) {
     var keys = this.loadFile(`./cmds/${module}/${file}`)
 ...
 checkPerms: function (msg, permLvl) {
  ...
  util.getSend(msg, '❌ | Tus permisos no son suficientes para usar ese comando... bruh | ❌');
  return false;
 },
 getCmd: function (arg) {
